  ...
  return (
    <div className="min-h-screen bg-[#080808] flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(6)].map((_, i) => (
          <div key={i} className="absolute w-1 h-1 bg-[#C9A84C] rounded-full animate-ping"
            style={{ left: `${15 + i * 15}%`, top: `${10 + (i % 4) * 20}%`, animationDelay: `${i * 0.5}s`, opacity: 0.3 }} />
        ))}
      </div>
      <div className="relative z-10 text-center max-w-lg w-full animate-fadeInUp">
        ...
        <div className="flex items-center justify-center gap-3">
          ...
          <span className="text-xs text-[#8A6E2F]">Redirecting to your vault in {countdown}s...</span>
        </div>
        <div className="mt-8 pt-6 border-t border-[#C9A84C]/15">
          <div className="flex justify-center gap-6">
            <Link to="/" className="text-xs tracking-[2px] uppercase text-[#C9A84C] hover:text-[#FFD97A] transition-colors font-cinzel">Home</Link>
            <Link to="/browse" className="text-xs tracking-[2px] uppercase text-[#C9A84C] hover:text-[#FFD97A] transition-colors font-cinzel">Browse</Link>
            <Link to="/sell" className="text-xs tracking-[2px] uppercase text-[#C9A84C] hover:text-[#FFD97A] transition-colors font-cinzel">Sell</Link>
          </div>
        </div>
      </div>
    </div>
