...
function getAdminEmails(c: any): string[] {
  const raw = typeof c.env.ADMIN_EMAILS === 'string' ? c.env.ADMIN_EMAILS : '';
  return raw
    .split(',')
  ...
  switch (eventType) {
    case 'user.created': {
      ...
      if (!existing) {
        ...
        const isAdminEmail = getAdminEmails(c).includes(normalizedEmail);
        ...
        console.log('Clerk: User created:', { id: clerkId, email, name, isAdminEmail });
      }
      break;
    }
