...
export const expertRouter = createRouter({
  ...
  submitApplication: publicQuery
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const appId = Number(result.insertId);

      // Auto-start outreach campaign for buyer leads
      const [outreachResult] = await db.insert(outreachCampaigns).values({
        applicationId: appId,
        userId: ctx.user?.id || null,
        itemName: input.itemName,
        category: input.category,
      ...
      const [campaign] = await db.select().from(outreachCampaigns).where(eq(outreachCampaigns.id, Number(outreachResult.insertId))).limit(1);
      ...
      return {
        ...
        reviewFee,
        outreachCampaignId: Number(outreachResult.insertId),
  ...
  runExpertReview: publicQuery.input(z.object({ applicationId: z.number() })).mutation(async ({ input }) => {
    ...
    for (const expert of experts) {
      ...
      const [existing] = await db.select().from(expertReviews).where(
        and(eq(expertReviews.applicationId, app.id), eq(expertReviews.expertId, expert.id))
      ).limit(1);
...
Be thorough and professional. Base your assessment on realistic market data.`;
      ...
      const [newReview] = await db.select().from(expertReviews).where(eq(expertReviews.id, Number(reviewResult.insertId))).limit(1);
...
Write a 3-paragraph professional consensus summary: (1) overall assessment, (2) detailed analysis of authenticity, value, and condition, (3) recommendation for the collector.`;
    ...
    const [consensus] = await db.select().from(expertConsensus).where(eq(expertConsensus.id, Number(consResult.insertId))).limit(1);
